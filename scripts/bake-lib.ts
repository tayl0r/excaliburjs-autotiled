...
export class TileRegistry {
  ...
  /** Look up the baked ID for an unflipped tile. Returns 0 if not registered. */
  getBakedId(tilesetIndex: number, tileId: number): number {
    const key = `${tilesetIndex}:${tileId}:0:0:0`;
    return this.map.get(key)?.bakedId ?? 0;
  }
...
export function registerAnimationFrames(
  metadata: ProjectMetadata,
  registry: TileRegistry,
): number {
  let count = 0;
  for (const ws of metadata.wangsets) {
...
export function generateIndex(
  ...
  const lines: string[] = [
    ...
    ...(oversizeTiles.length === 0
      ? ['  oversizeTiles: {} as Record<number, { atlasX: number; atlasY: number; sourceWidth: number; sourceHeight: number; renderOffsetX: number; renderOffsetY: number }>,']
      : [
          '  oversizeTiles: {',
          ...oversizeTiles.map(t =>
            `    ${t.bakedId}: { atlasX: ${t.atlasX}, atlasY: ${t.atlasY}, sourceWidth: ${t.sourceWidth}, sourceHeight: ${t.sourceHeight}, renderOffsetX: ${t.renderOffsetX}, renderOffsetY: ${t.renderOffsetY} },`,
          ),
  ...
  if (metadata && registry) {
    ...
    for (const ws of metadata.wangsets) {
      for (const wt of ws.wangtiles) {
        if (!wt.animation) continue;
        const baseBakedId = registry.getBakedId(wt.tileset ?? 0, wt.tileid);
        if (baseBakedId === 0) continue;
        const frameBakedIds = wt.animation.frames
          .map(f => f.tileId < 0 ? 0 : registry.getBakedId(f.tileset, f.tileId));
        lines.push(`  ${baseBakedId}: { frameDuration: ${wt.animation.frameDuration}, pattern: ${JSON.stringify(wt.animation.pattern)}, frames: [${frameBakedIds.join(', ')}] },`);
      }
    }
    lines.push('} as const;', '');
  }
...
Some tiles (e.g., 60x64 monster sprites) are larger than the standard ${TILE_SIZE}x${TILE_SIZE} grid size. These are packed in aligned blocks in the atlas and listed in \`atlas.oversizeTiles\`.
...
Oversized tiles are bottom-center anchored to their ${TILE_SIZE}x${TILE_SIZE} grid cell. Apply the render offsets when positioning:

\`\`\`typescript
const oversize = atlas.oversizeTiles[bakedId];
if (oversize) {
  sprite.x = cellX * ${TILE_SIZE} + oversize.renderOffsetX;
  sprite.y = cellY * ${TILE_SIZE} + oversize.renderOffsetY;
}
\`\`\`
